...
describe('Test index Component of addmeal component', () => {
  ...
  it('it should change display error message from redux error message props', () => {
  ...
  it('should should respond to lifeCycle Methods', () => {
    ...
    tree.instance().constructor.getDerivedStateFromProps({ errorMessage: { createMealError: '' }, successMessage: { createMealSuccess: '' } });
    wrapper.componentWillUnmount();
    expect(tree).toMatchSnapshot();
    tree.instance().constructor.getDerivedStateFromProps({ errorMessage: { createMealError: 'error' }, successMessage: { createMealSuccess: 'success' } });
    expect(tree).toMatchSnapshot();
  });
