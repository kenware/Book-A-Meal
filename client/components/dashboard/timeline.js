...
/**
  ...
  */
const Timeline = ({ notifics, upGrade, mostOrder }) => (
  <div>
    <ul className="timeline" >
          ...
          <div className="timeline-body">
            <ul>
              <li style={{ listStyle: 'none' }}>
                <img src={window.localStorage.getItem('image') !== 'null' ? window.localStorage.getItem('image') : 'image/profile.png'} className="user-img rounded-circle" alt="profil" />
              </li>
              <li>
      ...
      <li>
        ...
        <div className="timeline-panel bg-light">
          <div className="timeline-heading">
            <h5 className="timeline-title m-text l-margin">Most Ordered Meal</h5>
          </div>
          <div className="timeline-body">
            <ul style={{ overflow: 'scroll', height: '300px' }}>
              {mostOrder.map(meal =>
                (
                  <li style={{ marginTop: '1rem' }} key={meal.name}>
                    <div className="p-color">{meal.name}</div>
                  </li>
                ))}
            </ul>
          </div>
